...
extension Dictionary {
     ...
     */
    open func convertToModel<T: Decodable>() -> T? {
        do {
     ...
     */
    open func toModel<T: Decodable>(key: String?) -> T {
    ...
    /**
     ...
     - Returns: dictionary.
     */
    open func merge(with: [Key: Value]?) -> [Key: Value]? {
        var copy = self
        for (key, val) in with! {
...
extension Dictionary where Key: Any, Value: Any {
     ...
     */
    open func getValue(forKeyPath components: [Any]) -> [String: AnyObject] {
